...
$req = $conne->prepare("select count(reference) as nbgagne,extract(year from date_demande) as annee from sead.etude where statut=5 group by annee ORDER BY annee");
//$req2 = $conne->prepare("select count(reference) as nbperdu,extract(year from date_demande) as annee from sead.etude where (statut<=4 or statut >=6) group by annee ORDER BY annee");
 ...
 // $req2->execute();
   ...
   //print $row['COUNT(nom)'];
	$dataPoi = array("y" => $row['nbgagne'],"label" => $row['annee']);
