...
//Data
        ...
        $sql = "SELECT DISTINCT case when position('-' in sead.etude.nom)>0 
                ...
                else split_part(sead.etude.nom,' - ',1) end as nom,etude.date_reception,etude.date_remise_reelle,type.libelle AS libelle1,statut.libelle AS libelle2 
        FROM sead.etude,sead.type,sead.statut WHERE (statut <= 2 or statut >= 8) AND type.id_type=etude.type AND statut.id_statut=etude.statut AND date_demande BETWEEN '$debut' AND '$fin'";
...
$col = 0;
    ...
    $col += 1;
    $objPHPExcel->getActiveSheet()->setCellValueExplicitByColumnAndRow($col, $row, "Type de traitement");
    $col += 1;
    $objPHPExcel->getActiveSheet()->setCellValueExplicitByColumnAndRow($col, $row, "Statut");
    $col += 1;
...
//        exit();
            ...
            $col += 1;
            $objPHPExcel->getActiveSheet()->setCellValueExplicitByColumnAndRow($col, $row, $value['libelle1']);
            $col += 1;
            $objPHPExcel->getActiveSheet()->setCellValueExplicitByColumnAndRow($col, $row, $value['libelle2']);
            $col += 1;
...
$nom_fichier = "encours_" . date("dmY") . ".xlsx";
